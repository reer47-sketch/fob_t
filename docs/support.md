...
### 브리딩 캘린더
- 사이드바 메뉴의 "브리딩 캘린더"(/calendar)에서 접근
- 월 뷰 / 주 뷰 전환 가능
- 표시되는 이벤트 종류:
  - 입양(초록): 해당 날짜에 입양 등록된 개체
  - 해칭(초록): 해당 날짜에 해칭 등록된 개체
  - 판매(파랑): 해당 날짜에 판매된 개체
  - 폐사(회색): 해당 날짜에 폐사 처리된 개체
  - 메이팅(분홍): 해당 날짜에 메이팅 기록
  - 산란(노랑): 해당 날짜에 산란 기록
  - 부화(하늘): 해당 날짜에 부화 처리된 알
  - 부화 예정(보라 점선): 예상 부화일이 설정된 알
  - 태스크(주황): 사용자가 등록한 할 일
- 날짜를 클릭하면 우측 패널에 해당 날짜 상세 내역 표시
- 신규 등록된 개체 옆 링크 아이콘 클릭 시 해당 개체 상세 페이지로 이동
- 7일 이상 피딩 기록이 없는 개체는 상단에 경고 배너로 표시
- 태스크 기능: 청소, 렉사 설치, 먹이 준비, 건강 체크, 기타 카테고리로 할 일 등록/완료/삭제
- Google Calendar 연동: 부화 예정일 및 태스크를 구글 캘린더로 동기화 가능 (연동 버튼 클릭)
